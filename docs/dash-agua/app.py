...
# ==========================================
# 2. INICIALIZAR APP CON BOOTSTRAP
# ==========================================
app = dash.Dash(__name__, external_stylesheets=[dbc.themes.BOOTSTRAP, dbc.themes.BOOTSTRAP])
    ...
    dbc.Row([
        dbc.Col([
            create_card("6.4 Estadísticas Descriptivas", [
                ...
                dbc.Alert("""
                Las estadísticas descriptivas permiten conocer el comportamiento general de las variables 
                numéricas. Se observa que las características presentan diferentes rangos y niveles de dispersión.
                """, color="info", className="mt-3")
            ], color="warning")
        ])
    ], className="mb-4"),
    ...
    dbc.Row([
        dbc.Col([
            create_card("6.7 Matriz de Correlación Completa", [
                ...
                dbc.Alert("""
                La matriz de correlación permite visualizar las relaciones lineales entre todas las variables.
                """, color="info", className="mt-3")
            ], color="dark")
        ])
    ], className="mb-4"),
...
# ==========================================
# 6. CALLBACK PARA CAMBIAR DE SECCIÓN
# ==========================================
@app.callback(
    ...
    try:
        if tipo_grafico == 'histogram':
            ...
            fig = px.box(df, x='Potability', y=variable,
                        ...
                        color_discrete_map={0: '#8B4513', 1: '#4169E1'},
                        points='outliers')
            # Actualizar etiquetas del eje x (CORREGIDO: update_xaxes con 's')
        ...
        import traceback
        traceback.print_exc()
        # Retorna un gráfico vacío si hay error
        return go.Figure()
...
def actualizar_scatter(col_x, col_y, tipo):
    try:
        # IMPORTANTE: Eliminar NaN para que funcione la línea de tendencia
        ...
        import traceback
        traceback.print_exc()
        # Retorna gráfico vacío con mensaje de error
        fig = go.Figure()
